document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    function getFieldError(field) {
      ...
      if (
        (name.toLowerCase().includes('name') || field.dataset.validate === 'name') &&
        value !== '' &&
        value.length < 2
      ) {
    ...
    function showFieldError(field, message) {
      ...
      const wrapper = field.closest('.form-group, .field-wrapper, .input-wrapper');
      if (wrapper) {
    ...
    function clearFieldError(field) {
      ...
      const wrapper = field.closest('.form-group, .field-wrapper, .input-wrapper');
      const container = wrapper || field.parentElement;
      const existingError = container
        ? container.querySelector('.form-error-message')
        : null;
    ...
    function showFormSuccess() {
      let successEl = contactForm.querySelector('.form-success-message');
      if (!successEl) {
    ...
    const validatableFields = contactForm.querySelectorAll(
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      if (!isValid) {
        ...
        if (firstError) {
          const navbarEl = document.querySelector('.navbar, nav, header');
          const offset = navbarEl ? navbarEl.offsetHeight + 16 : 16;
          const top =
            firstError.getBoundingClientRect().top + window.pageYOffset - offset;
  ...
  const animatableEls = document.querySelectorAll(OBSERVED_SELECTORS.join(', '));
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'b1a34055-d478-439b-b2eb-ab657c316e7c',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'b1a34055-d478-439b-b2eb-ab657c316e7c';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'b1a34055-d478-439b-b2eb-ab657c316e7c';
